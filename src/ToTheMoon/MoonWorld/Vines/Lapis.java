...
public class Lapis extends BlockPopulator {
	...
	public void populate(World world, Random r, Chunk chunk) {
		...
		for (int i = 1; i < 15; i++) {
			if (r.nextInt(100) < this.fileConfig.getInt("ToTheMoon.WorldGeneration.LapisSpawnPercentage")) {
				int X = r.nextInt(15);
				int Z = r.nextInt(15);
				...
				if (chunk.getBlock((X + 16) % 16, Y, (Z + 16) % 16).getType() == Material.END_STONE) {
					...
					while (isStone) {
						try {
							chunk.getBlock((X + 16) % 16, Y, (Z + 16) % 16).setType(Material.LAPIS_ORE);
							if (r.nextInt(100) < 40) {
								switch (r.nextInt(5)) {
									case 0:
										X++;
								...
								isStone = (chunk.getBlock((X + 16) % 16, Y, (Z + 16) % 16).getType()
									== Material.END_STONE
									&& chunk.getBlock((X + 16) % 16, Y, (Z + 16) % 16).getType() != Material.LAPIS_ORE);
								continue;
							}
							isStone = false;
						}
 ...
 * Vines\Lapis.class Java compiler version: 7 (51.0) JD-Core Version: 1.1.3
 */